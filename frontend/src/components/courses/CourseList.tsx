                ...
                <AnimatePresence mode="popLayout">
                    {filteredCourses.map((course, idx) => (
                        ...
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-start md:items-center gap-6">
                                    <div className="shrink-0 w-20 h-20 rounded-3xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 flex flex-col items-center justify-center font-black shadow-xl shadow-slate-900/10 dark:shadow-white/5 group-hover:scale-110 transition-transform duration-500">
                                        <span className="text-2xl">{course.code.match(/\d+/) ? course.code.match(/\d+/)?.[0] : "???"}</span>
                                        <span className="text-[10px] uppercase tracking-tighter opacity-70">Unit {course.units || 3}</span>
                                    </div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {course.level && (
                                                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md flex items-center gap-1">
                                                    <Clock size={10} /> {course.level.name}
                                                </span>
                                            )}
                                        </div>
                            ...
                            {/* Decorative line */}
                            <div className="absolute bottom-0 left-10 right-10 h-1 bg-gradient-to-r from-transparent via-blue-500/20 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700" />
                        </motion.div>
                    ))}
                </AnimatePresence>
            </div>
